...
SelectFont=()=>{
  var fonts=['Fira Sans','Noto Sans','Playfair Display','Roboto Condensed','Ubuntu','Tenali Ramakrishna','Poppins','Abril Fatface','Bangers','Fredoka One','Pacifico','Righteous'];
  if(this.state.i===11){
    this.setState({fnts:"'"+fonts[this.state.i]+"',sans-serif",i:0})
  }
  else if(this.state.i>6){
    this.setState({fnts:"'"+fonts[this.state.i]+"',cursive",i:this.state.i+1})}
  else{
      this.setState({fnts:"'"+fonts[this.state.i]+"',sans-serif",i:this.state.i+1})}
}
render(){
    return (
      <div className="justify-content-center px-2 py-2">
      <div className="box" id="boxG" style={{background:this.state.colors,height:"520px"}}><h5 className="text-light px-5 py-5" style={{fontFamily:this.state.fnts,fontSize:this.state.textSizes+"px",alignItems:"center",justifyContent: "center",textShadow:"2px 2px 8px #000000",display: "flex"}}>{this.state.textData}</h5></div><br/>
      ...
      <textarea ref={(textarea) => this.textArea = textarea} style={{opacity:"0",width:"0",height:"0"}} value={"background:"+this.state.colors+";"}/>
    </div>
  );
}}
